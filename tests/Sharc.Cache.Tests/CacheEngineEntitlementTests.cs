...
public sealed class CacheEngineEntitlementTests : IDisposable
{
    ...
    [Fact]
    public void Constructor_EnvVarMasterKey_UsedAsFallback()
    {
        var key = new byte[32];
        RandomNumberGenerator.Fill(key);
        var base64 = Convert.ToBase64String(key);

        Environment.SetEnvironmentVariable("SHARC_CACHE_KEY", base64);
        try
        {
            using var engine = new CacheEngine(
                new CacheOptions
                {
                    SweepInterval = TimeSpan.Zero,
                    EnableEntitlement = true,
                    // No MasterKey or MasterKeyProvider — should fall back to env var
                },
                _provider);

            _provider.CurrentScope = "tenant:acme";
            engine.Set("k1", [1, 2, 3], new CacheEntryOptions { Scope = "tenant:acme" });
            Assert.Equal(new byte[] { 1, 2, 3 }, engine.Get("k1"));
        }
        finally
        {
            Environment.SetEnvironmentVariable("SHARC_CACHE_KEY", null);
        }
    }

    [Fact]
    public void Constructor_MasterKeyProvider_TakesPrecedenceOverEnvVar()
    {
        var providerKey = new byte[32];
        RandomNumberGenerator.Fill(providerKey);

        var envKey = new byte[32]; // different key
        RandomNumberGenerator.Fill(envKey);
        Environment.SetEnvironmentVariable("SHARC_CACHE_KEY", Convert.ToBase64String(envKey));
    ...
    [Fact]
    public void Constructor_NoKeySource_Throws()
    {
        Environment.SetEnvironmentVariable("SHARC_CACHE_KEY", null);

        var ex = Assert.Throws<ArgumentException>(() => new CacheEngine(
            new CacheOptions
            {
                SweepInterval = TimeSpan.Zero,
                EnableEntitlement = true,
