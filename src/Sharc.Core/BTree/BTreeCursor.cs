...
internal sealed class BTreeCursor : IBTreeCursor
{
    ...
    public bool Seek(long rowId)
    {
        ...
        else
        {
            ...
            if (MoveToNextLeaf())
            {
                _currentCellIndex = 0; // DescendToLeftmostLeaf sets -1, but we need 0 to read immediately
                ParseCurrentLeafCell();
                return false;
            }
            else
    ...
    private void DescendToLeftmostLeaf(uint pageNumber)
    {
        while (true)
        {
            ...
            if (header.CellCount == 0)
            {
                ...
                // Note: Pushing 0 here for empty page creates infinite loop potential in Scan, 
                // but strictly following original logic for now (assuming valid pages)
    ...
    private bool MoveToNextLeaf()
    {
        while (_stack.Count > 0)
        {
            ...
            if (nextCellIndex < header.CellCount)
            {
                ...
                // The 'pointers' array contains offsets to cells.
                // We need to read the LeftChild from the cell at nextCellIndex.
                int cellOffset = pointers[nextCellIndex];
                
                // Read Left Child
                uint leftChild = BinaryPrimitives.ReadUInt32BigEndian(
                    interiorPage[cellOffset..]);
