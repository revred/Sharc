...
Yes! Sharq supports `SELECT`, `FROM`, `WHERE`, `JOIN` (INNER/LEFT/RIGHT/FULL OUTER/CROSS), `ORDER BY`, `GROUP BY`, `HAVING`, `LIMIT`, `OFFSET`, `UNION`/`INTERSECT`/`EXCEPT`, Cotes (`WITH ... AS`), and parameterized queries (`$param`). You only need the arrow syntax for Graph Engine features.

> **Note:** `CASE` expressions and window functions (`OVER`, `PARTITION BY`) are parsed but not yet executable.
